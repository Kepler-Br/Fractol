...
t_main_loop *t_main_loop_create(char *title, cl_uint2 window_geometry, struct s_state *state)
{
	...
	if(!(instance.window = mlx_new_window(instance.mlx, window_geometry.x, window_geometry.y, title)))
	{
		write(1, "t_main_loop_create: cannot init mlx window.", 43);
		exit(1);
	}
	instance.window_geometry = window_geometry;
	object->mlx_instance = instance;
