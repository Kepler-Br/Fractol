...
static void loop(struct s_state *this)
{
    ...
    this_str->mandelbulb_power = lerpf(this_str->mandelbulb_power, this_str->target_mandelbulb_power, 0.1f);
...
t_state		*t_raymarch_state_create(t_mlx_instance mlx_instance, char *fragment)
{
	...
	{
		write(1, "t_raymarch_state_create: cannot allocate t_raymarch_struct.\n", 60);
		exit(1);
	}
